...
int http_joint_file_response(int accept_socket, HTTP_RECV_INFO *http_recv_info_p  )
{
    ...
    // ワーク領域確保
    // -----------------------
    joint_file_info_p = (JOINT_FILE_INFO_T*)malloc( sizeof(JOINT_FILE_INFO_T) );
...
static int http_joint_file_send(int	accept_socket, JOINT_FILE_INFO_T *joint_file_info_p, off_t content_length, off_t range_start_pos )
{
    ...
    while ( 1 )
    {
        ...
        if ( data_send_len != (int)buf_charge_size )
        {
...
static int analyze_svi_file(unsigned char *svi_filename, JOINT_FILE_INFO_T *joint_file_info_p )
{
    ...
    unsigned char	svi_filepath[FILENAME_MAX];
    unsigned char	first_filename[FILENAME_MAX];
    unsigned char	series_filename[FILENAME_MAX];
    ...
    // -------------------------------------
    // 最後の'\'を検索して、'/'に置換
    p = (unsigned char*)strrchr(read_filename, '\\');
    if ( p != NULL )
    {
        *p = '/';
    }
    ...
    for ( i=1; i<JOINT_MAX; i++ )
    {
        ...
        p = (unsigned char*)strrchr( series_filename, '.');
        if ( p != NULL )
        {
