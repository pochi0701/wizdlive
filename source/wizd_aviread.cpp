...
{
    DWORD *p = (DWORD*)ptr;
    avih->dwMicroSecPerFrame    = get_le_dword((unsigned char*)(p++));
    avih->dwMaxBytesPerSec      = get_le_dword((unsigned char*)(p++));
    avih->dwReserved1           = get_le_dword((unsigned char*)(p++));
    avih->dwFlags               = get_le_dword((unsigned char*)(p++));
    avih->dwTotalFrames         = get_le_dword((unsigned char*)(p++));
    avih->dwInitialFrames       = get_le_dword((unsigned char*)(p++));
    avih->dwStreams             = get_le_dword((unsigned char*)(p++));
    avih->dwSuggestedBufferSize = get_le_dword((unsigned char*)(p++));
    avih->dwWidth               = get_le_dword((unsigned char*)(p++));
    avih->dwHeight              = get_le_dword((unsigned char*)(p++));
    avih->dwScale               = get_le_dword((unsigned char*)(p++));
    avih->dwRate                = get_le_dword((unsigned char*)(p++));
    avih->dwStart               = get_le_dword((unsigned char*)(p++));
    avih->dwLength              = get_le_dword((unsigned char*)(p++));
}
void set_avish(AVIStreamHeader *avish, char *ptr)
{
    DWORD *p = (DWORD*)ptr;
    avish->fccType               = get_le_dword((unsigned char*)(p++));
    avish->fccHandler            = get_le_dword((unsigned char*)(p++));
    avish->dwFlags               = get_le_dword((unsigned char*)(p++));
    avish->dwReserved1           = get_le_dword((unsigned char*)(p++));
    avish->dwInitialFrames       = get_le_dword((unsigned char*)(p++));
    avish->dwScale               = get_le_dword((unsigned char*)(p++));
    avish->dwRate                = get_le_dword((unsigned char*)(p++));
    avish->dwStart               = get_le_dword((unsigned char*)(p++));
    avish->dwLength              = get_le_dword((unsigned char*)(p++));
    avish->dwSuggestedBufferSize = get_le_dword((unsigned char*)(p++));
    avish->dwQuality             = get_le_dword((unsigned char*)(p++));
    avish->dwSampleSize          = get_le_dword((unsigned char*)(p++));
    ...
    while (len > 0 && (chunklen = read_next_chunk(fp, &fcc)) > 0) {
        ...
        case SYM_STRF:
            ...
            if (avish->fccType == SYM_VIDS) {
                ...
                avish->dwReserved1 = get_le_dword((unsigned char*)&bi->biCompression);
            } else if (avish->fccType == SYM_AUDS) {
                /* save wFormatTag into avish->dwReserved1 :p */
                WAVEFORMATEX *wi = (WAVEFORMATEX*)ptr;
                avish->dwReserved1 = get_le_word((unsigned char*)&wi->wFormatTag);
            }
            free(ptr);
            break;
        default:
